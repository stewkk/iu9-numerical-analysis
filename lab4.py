...
def runge_kutta_step(y1, y2, h):
    k1y1, k1y2 = dydx(y1, y2)
    k2y1, k2y2 = dydx(y1 + h / 2 * k1y1, y2 + h / 2 * k1y2)
    k3y1, k3y2 = dydx(y1 + h / 2 * k2y1, y2 + h / 2 * k2y2)
    k4y1, k4y2 = dydx(y1 + h * k3y1, y2 + h * k3y2)
...
for i in range(1000):
    ...
    while x < b:
        ...
        error = abs(y1_h - y1_2h) / precise_order
        x_new = x + h
