...
\begin{titlepage}
	...
	\begin{center}
		\textbf{ОТЧЕТ} \\\textit{по лабораторной работе № 5\\по курсу <<Численные методы>>\\на тему: <<Метод наискорейшего спуска поиска минимума\\ функции многих переменных>>\\Вариант № 25} \\
	\end{center}
...
\begin{enumerate}
  \item Найти минимум функции двух переменных с точностью
        \( \varepsilon = 0.001 \), начиная итерации с точки \( x^0 \).
  \item Найти минимум аналитически.
  \item Сравнить полученные результаты.
\end{enumerate}

\textbf{Индивидуальный вариант:} \\
Функция: \( x + 2 y + 4 (1 + x^2 + y^2)^{0.5} \), начальная точка: \( x^0 = (0, 0) \).

\section{Теоретический раздел}

Метод наискорейшего спуска является итерационным. Пусть для функции
\( f(x_1, \dots, x_n) \) на \( k \)-м шаге имеем некоторое приближение к
минимуму \( x^k = (x_1^k, \dots, x_n^k) \). Рассмотрим функцию одной переменной:

\[
  \phi_k(t) = f\left(x_1^k - t \frac{\partial f}{\partial x_1}(x^k), \dots, x_n^k - t \frac{\partial f}{\partial x_n}(x^k)\right) = f(x^k - t \cdot \text{grad} f(x^k)).
\]

Функция \( \phi_k(t) \) есть ограничение исходной функции \( f(x) \) на прямую
градиентного (наискорейшего) спуска, проходящую через точку \( k \)-го
приближения \( x^k \).

Минимум функции \( \phi_k(t) \) можно найти любым методом одномерной
оптимизации. Обозначим эту точку минимума через \( t^* \). Теперь для следующего
приближения к точке экстремума полагаем:

\[
  x^{k+1} = x^k - t^* \cdot \text{grad} f(x^k) = \left(x_1^k - t^* \frac{\partial f}{\partial x_1}(x^k), \dots, x_n^k - t^* \frac{\partial f}{\partial x_n}(x^k)\right).
\]

Процесс поиска минимума продолжается до тех пор, пока
\( \|\text{grad} f(x^k)\| = \max_{1 \leq i \leq n} \left|\frac{\partial f}{\partial x_i}(x^k)\right| \)
не станет меньше допустимой погрешности \( \varepsilon \).
...
(-0.3014899578264943, -0.6029799156529886) = 3.316624792962804
analytical f((-0.30151134457776363, -0.6030226891555273)): 3.3166247903554
diff f(x, y): 0.0000000026
diff: (0.0000214, 0.0000428)
  \end{minted}

\section{Вывод}

В ходе выполнения лабораторной работы был реализован метод наискорейшего спуска
поиска минимума функции многих переменных. Реализация была выполнена в виде
программы на языке Python и проведено сравнение результатов работы алгоритма с
найденным аналитически минимумом.
